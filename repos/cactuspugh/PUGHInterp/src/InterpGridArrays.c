                              ...
                              CCTK_POINTER const output_arrays [])
{
  ...
  if (CCTK_CoordSystemDim (coord_system_name) < N_dims)
  {
    CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                ...
                "coordinates (dim = %d)", coord_system_name, (int)N_dims);
    return (UTIL_ERROR_BAD_INPUT);
  }
  ...
  for (i = retval = 0; i < N_input_arrays; i++)
  {
    ...
    if (group_data.dim > N_dims)
    {
      CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                  ...
                  (int)input_array_indices[i], coord_system_name);
      retval = UTIL_ERROR_BAD_INPUT;
      continue;
    }
    ...
    if (! input_arrays[i])
    {
      ...
      CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                  ...
                  (int)input_array_indices[i], (int)input_array_time_levels[i]);
      free (fullname);
      retval = UTIL_ERROR_BAD_INPUT;
      continue;
    }
