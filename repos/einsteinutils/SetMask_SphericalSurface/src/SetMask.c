...
void SetMask_SphericalSurface (CCTK_ARGUMENTS)
{
  ...
  if (!mask) {
    CCTK_VERROR("No such variable, or no storage enabled: '%s'",
                SetMask_MaskName);
  }
  ...
  for (int smi = 0; smi < 10; smi++)
  {
    ...
    if (!(sfi < nsurfaces)) {
        CCTK_VERROR("Surface index %d for mask %d exceeds index allowed by SphericalSurface::nsurfaces which is %d",
                    sfi, smi, nsurfaces);
    }
...
void SetMask_ParamCheck (CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;

  int maskidx = CCTK_VarIndex(SetMask_MaskName);
  if (maskidx < 0) {
    CCTK_VPARAMWARN("Grid function '%s used for %s::SetMask_MaskName' does not exist",
                    SetMask_MaskName, CCTK_THORNSTRING);
  }

  // technically storage could be enabled later, though I cannot think of a
  // good reason why one would do this
  int masktimelevels = CCTK_ActiveTimeLevels(cctkGH, SetMask_MaskName);
  if (masktimelevels < 1) {
    CCTK_VPARAMWARN("Grid function '%s' used for %s::SetMask_MaskName has no storage",
                    SetMask_MaskName, CCTK_THORNSTRING);
  }

  for (int smi = 0; smi < 10; smi++) {
    CCTK_INT sfi = sf_IdFromName(SetMask_SurfaceIndex[smi],
                                 SetMask_SurfaceName[smi]);
    if(!CCTK_EQUALS(SetMask_SurfaceName[smi], "") && sfi < 0) {
        CCTK_VPARAMWARN("Spherical surface '%s' does not exist",
                        SetMask_SurfaceName[smi]);
    }

    if (!(sfi < nsurfaces)) {
        CCTK_VPARAMWARN("Surface index %d for mask %d exceeds index allowed by SphericalSurface::nsurfaces which is %d",
                        sfi, smi, nsurfaces);
    }
  }
}
