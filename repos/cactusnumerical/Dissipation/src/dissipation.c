...
apply (int const varindex, char const * const optstring, void * const arg)
{
  ...
  for (d=0; d<cctk_dim; ++d) {
    if (cctk_nghostzones[d] < (order+1)/2) {
      CCTK_ERROR ("This thorn requires at least (order+1)/2 ghost zones");
    }
  }
  ...
  if (rhsindex < 0) {
    ...
    assert (fullvarname);
    CCTK_VERROR ("There is no RHS variable registered with MoL for the evolved variable \"%s\"",
                 fullvarname);
    free (fullvarname);
  }
  assert (rhsindex >= 0);
  ...
  if (verbose) {
    ...
    assert (fullvarname);
    assert (fullrhsname);
    CCTK_VINFO ("Applying dissipation to \"%s\" (RHS \"%s\")",
  ...
  CCTK_FNAME(apply_dissipation)
    (varptr, rhsptr, &cctk_ash[0], &cctk_ash[1], &cctk_ash[2],
     &cctk_lsh[0], &cctk_lsh[1], &cctk_lsh[2], dx, &order, epsdisA);
}
