...
{
  DECLARE_CCTK_ARGUMENTS_setup_epsdis;
  ...
  if (verbose) {
    CCTK_VINFO("Setting up spatially varying dissipation at T=%g",
               (double)cctk_time);
  }
  ...
  if (extra_dissipation_in_horizons && cctk_iteration%update_ah_every == 0)
  {
    ...
    for (s=0;s<MAXSURFNUM;s++)
    {
      ...
      if (surface_number[s]<0 || horizon_number[s]<=0) {
        CCTK_VWARN (1, "Invalid specification for horizon %d", s);
        continue;
      }
      
      if (! sf_valid[surface_number[s]]) {
        if (verbose) {
          CCTK_VINFO("Invalid Surface: s=%d, sf_va=%d, surf_no=%d, ah_no=%d",
