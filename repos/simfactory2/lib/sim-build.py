...
def PrepConfiguration(configName):
    ...
    if simenv.OptionsManager.HasOption('makejobs'):
        DefineDatabase.Set('MAKEJOBS', simenv.OptionsManager.GetOption('makejobs'))
    elif machineEntry.HasKey('makejobs'):
        DefineDatabase.Set('MAKEJOBS', machineEntry.GetKey('makejobs'))
    else:
        DefineDatabase.Set('MAKEJOBS', 1)
    ...
    if optionlist is None or not os.path.exists(optionlist):
    ...
    hasOutdatedConfig = (not hasCompleteConfig) or (storedOptionSettings is None) or (storedOptionSettings != optionSettings)
    ...
    if hasOutdatedConfig or build_reconfig:
        ...
        if storedOptionSettings is not None:
