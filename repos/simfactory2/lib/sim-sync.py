...
def CompileCommand(machineName, rsyncInfo, paths, oldRsync=False):
    ...
    mreq_keys = ['user', 'allocation', 'sourcebasedir', 'hostname', 'rsynccmd', 'rsyncopts', 'sshcmd', 'sshopts']
    simlib.VerifyKeys(machineEntry, mreq_keys)

    DefineDatabase.Set('USER', machineEntry.user)
    DefineDatabase.Set('ALLOCATION', machineEntry.allocation)
...
#    sshcmd = DefineDatabase.SubAll(sshcmd)
    ...
    rsyncoptions = [
        ...
        '--links',
        '--partial',
        #'--perms',   # Cannot use --perm on Graham
        '--progress',
        '--recursive',
        '--sparse',
        '--stats',
    ...
    cmd = "%(rsynccmd)s --rsh=%(sshcmd)s --rsync-path=%(entry_rsynccmd)s %(defaultrsyncoptions)s %(rsyncopts)s %(entry_rsyncopts)s %(arguments)s %(rsyncfiles)s %(fullpath)s" % {
         'rsynccmd': rsynccmd, 'sshcmd': simlib.QuoteSafe(sshcmd),
         'entry_rsynccmd': simlib.QuoteSafe(machineEntry.rsynccmd),
         'rsyncopts': rsyncopts,
         'entry_rsyncopts': machineEntry.rsyncopts,
         'arguments': arguments, 'defaultrsyncoptions': " ".join(rsyncoptions),
         'rsyncfiles':  " ".join(rsyncfiles), 'fullpath': fullpath}
    ...
    cmd = "cd %s && { %s; } && { %s && %s ; }" % (simlib.QuoteSafe(local_sourcedir), localsshsetup, mkdircmd, cmd)
