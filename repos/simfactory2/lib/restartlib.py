...
class SimulationLog:
    ...
    def Open(self):
        ...
        try:
            ...
            fatal("Could not open simulation log file \"%s\" for writing: %s" % (self.LogPath, e))
...
def CleanupRestarts():
    ...
    if simenv.LocalMachine is None:
        return
    ...
    for sim in simlib.GetSimulations():
        ...
        if sim_create_time is not None and (current_time - sim_create_time) < 60:
        ...
        if mark_time is not None and (current_time - mark_time) < 30:
    ...
    if ret is not None:
        ...
        for line in lines:
            ...
            if matches is not None:
                ...
                # holding_pattern
                if holding_pattern is not None:
                    if re.search(holding_pattern, line):
                        status = 'H'
                        matched.append(holding_pattern)

        # Python2 will compare None with integer using operator >, Python3 will not
        if matches is None:
            matches = 0
...
def GetExecutable():
    ...
    if not(simlib.DirectoryExists(configPath)):
...
def CopyFileWithCaching(srcfile, destdir, cachedir):
    ...
    if not(os.path.exists(cachedir)):
        try:
            ...
            fatal("could not create cache directory '%s': %s" % (cachedir, e))
    ...
    if not(os.path.exists(cachefile)):
        ...
        try:
            ...
            fatal("Could not copy %s to %s: %s" % (srcfile, dstfile, e))
    ...
    if not eq:
        ...
        if os.path.exists(cachefile):
            try:
                ...
                warning("could not remove existing cached executable '%s': %s" % (cachefile, e))
...
def CreateRestartSkeleton(simulationName):
    ...
    if simlib.DirectoryExists(internaldir):
