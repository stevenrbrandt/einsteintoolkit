...
class SimRestart:
    ...
    def ClearMark(self, label="timestamp"):
        # first try for existence of just the "label" file, if this fails try
        # old-behaviour of encoding the timestamp in the file name
        try:
            # remove existing mark
            path = simlib.BuildPath(self.InternalDir, label)
            os.unlink(path)
        except OSError as e:
            if e.errno == errno.ENOENT:
                expression = "%s-([0-9\\.]+$)" % label
                for fptr in os.listdir(self.InternalDir):
                    matches = re.match(expression, fptr)
    ...
    def mark(self, label="timestamp"):
        ...
        np = simlib.BuildPath(self.InternalDir, label)
        try:
            with open(np, "w") as fptr:
              fptr.write("%s\n" % timestamp)
        except OSError as e:
    ...
    def GetMarkTime(self, label="timestamp"):
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                ...
                fatal("Unable to read %s file %s, %s" % (label, np, e))
    ...
    def makeActive(self):
        ...
        try:
            ...
            fatal("unable to activate simulation %s, with restart id %s: %s" % (self.SimulationName, self.RestartID, e))
    ...
    def submit(self, submitScript, chainedJobId=None, chainedRestartId=None):
        ...
        if UseChaining and chainedJobId is None:
    ...
    def userRun(self, simulationName, debug=False):
        ...
        # set up pipes to capture stdout and stderr and write them to log files
        ...
        try:
            ...
            fatal("Failed to open error log file '%s': %s" % (errfile, e))
        try:
            outfh = open(outfile, "wb")
        except IOError as e:
            fatal("Failed to open output log file '%s': %s" % (outfile, e))

        # starting with python 3.4 pipes are non-inheritable though one would
        # hope that the dup() below which dups them into fd 0 and 1 keeps them
        # around in those two fds. Windows will close them before I have a
        # chance to up them
        # (https://docs.python.org/3/library/os.html#inheritance-of-file-descriptors).
        err_read, err_write = os.pipe()
        out_read, out_write = os.pipe()
        fcntl.fcntl(err_read, fcntl.F_SETFL, os.O_NONBLOCK)
        fcntl.fcntl(out_read, fcntl.F_SETFL, os.O_NONBLOCK)

        pid = os.fork()
        if pid > 0:
            os.close(err_write)
            err_read = os.fdopen(err_read, "rb")
            os.close(out_write)
            out_read = os.fdopen(out_read, "rb")
            done, runscript_rc = os.waitpid(pid, os.WNOHANG)
            while not done:
                rd, wr, ex = select.select([err_read, out_read], [], [])
                if err_read in rd :
                    err_txt = err_read.read()
                    # use os.write to be able to write byte stream to stdout
                    # instead of string
                    os.write(sys.stderr.fileno(), err_txt)
                    errfh.write(err_txt)
                if out_read in rd :
                    out_txt = out_read.read()
                    # use os.write to be able to write byte stream to stderr
                    # instead of string
                    os.write(sys.stdout.fileno(), out_txt)
                    outfh.write(out_txt)
                done, runscript_rc = os.waitpid(pid, os.WNOHANG)
            # get last bit of output possibly in pipes
            rd, wr, ex = select.select([err_read, out_read], [], [])
            if err_read in rd :
                err_txt = err_read.read()
                # use os.write to be able to write byte stream to stdout
    ...
    def run(self, debug=False):
        ...
        if testsuite:
            new_properties['NUM_PROCS'] = "${TESTSUITE_NPROCS}"
        else:
        ...
        if testsuite:
            ...
            # under TEST
            testsuitecmds = []
            testsuitecmds.append("rm -f output-0000-active")
            testsuitecmds.append("ln -s . output-0000-active")
            testsuitecmds.append("mkdir -p SIMFACTORY")
            testsuitecmds.append("TESTSUITE_PARFILE=$parfile TESTSUITE_NPROCS=$nprocs %s" % preparedRunScript)
            os.environ["CCTK_TESTSUITE_RUN_COMMAND"] = " && ".join(testsuitecmds)
            os.environ["CCTK_TESTSUITE_RUN_PROCESSORS"] = str(self.Properties.numprocs)

            MakeDefineDatabase = simsubs.DefineDatabase()
            MakeDefineDatabase.Set('MAKEJOBS', "1") # cannot run tests in parallel using make
            makecmd = MakeDefineDatabase.SubAll(machineEntry.GetKey('make'))
            cmd = "%s %s-testsuite PROMPT=no" % (makecmd, self.Properties.configuration)
        else:
        ...
        # Python does not set LC_XXX so it's locale specific date defaults to "C"
        old_lc_time = locale.getlocale(locale.LC_TIME)
        try: # Python aborts for unknown locales, so we trap these
            locale.setlocale(locale.LC_TIME, "") # respect LC_TIME settings
        except locale.Error:
            pass
        display("Simfactory Done at date: %s" % time.strftime("%c"))
        try:
    ...
    def show_output(self):
        ...
        if exechost is None and job_status == 'R':
        ...
        if job_status == 'R':
            ...
            if simenv.OptionsManager.GetOption('follow'):
                ...
                pid = simlib.ExecuteReplaceCommand(followcmd, background=True)
                retinfo = os.waitpid(pid, os.WNOHANG)
                while(retinfo[0] == 0):
                    time.sleep(60)
                    job_id = self.GetJobId()
                    job_status = 'U'
    ...
    def finish(self, allowForce=False):
        ...
        try:
            ...
            fatal("cannot clean up simulation %s, restart id %s, unable to remove -active symlink %s: %s" % (self.SimulationName, self.RestartID, active_dir, e))
