...
class OptionManager:
    ...
    def BuildOptionString(self, stripArguments=None):
        ...
        for key in keys:
            if stripArguments is not None and key in stripArguments:
                continue
    ...
    def AddOption(self, op, option, sdict):
        ...
        if sdict['negatable'].value == 'yes' and 'short' in sdict:
        ...
        if sdict['arg'].value == "yes":
            ...
            if 'multiarg' in sdict and sdict['multiarg'].value == "yes":
                optstring = self.AddOptionToOptionString(optstring, 'action', 'append')
            if 'defeult' in sdict:
                optstring = self.AddOptionToOptionString(optstring, 'default', sdict['default'].value)
        else:
            ...
            if 'default' in sdict:
                # this cannot use AddOptionToOptionString since the function adds quotes around value
