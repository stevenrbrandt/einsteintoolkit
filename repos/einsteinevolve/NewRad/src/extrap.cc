                    ...
                    CCTK_REAL* restrict const var)
{
  ...
  for (int k=lmin[2]; k!=lmax[2]; k+=idir[2]) {
    for (int j=lmin[1]; j!=lmax[1]; j+=idir[1]) {
      for (int i=lmin[0]; i!=lmax[0]; i+=idir[0]) {
      ...
      for (int dir1=-1; dir1<=+1; ++dir1) {
        for (int dir0=-1; dir0<=+1; ++dir0) {
          ...
          if (nnz == ifec) {
            ...
            // at least one boundary face is a symmetry boundary
            bool any_symbnd = false;
            ...
            for (int d=0; d<3; ++d) {
              ...
              case -1:
                ...
                any_symbnd = any_symbnd or is_symbnd[2*d+0];
              ...
              case +1:
                ...
                any_symbnd = any_symbnd or is_symbnd[2*d+1];
            ...
            if (have_bnd and not any_symbnd and any_physbnd and all_not_ipbnd) {
              extrap_kernel (cctkGH, imin, imax, bmin, bmax, dir, var);
            }
