  ...
  for(k = 0 ; k < GRHydro_stencil*mol_substep ; ++k) {
    for(j = 0 ; j < cctk_ash[1] ; ++j) {
      for(i = 0 ; i < cctk_ash[0] ; ++i) {
        dens[CCTK_GFINDEX3D(cctkGH, i,j,k)] = -1e100
        Scon[CCTK_VECTGFINDEX3D(cctkGH, i,j,k,0)] = -1e100
        Scon[CCTK_VECTGFINDEX3D(cctkGH, i,j,k,1)] = -1e100
        Scon[CCTK_VECTGFINDEX3D(cctkGH, i,j,k,2)] = -1e100
        tau[CCTK_GFINDEX3D(cctkGH, i,j,k)] = -1e100
        if(do_mhd) {
          Bcons[CCTK_VECTGFINDEX3D(cctkGH, i,j,k,0)] = -1e100
          Bcons[CCTK_VECTGFINDEX3D(cctkGH, i,j,k,1)] = -1e100
          Bcons[CCTK_VECTGFINDEX3D(cctkGH, i,j,k,2)] = -1e100
        }
        if(do_clean_divergene)
          psidc[CCTK_GFINDEX3D(cctkGH, i,j,k)] = -1e100
      }
    }
  }
