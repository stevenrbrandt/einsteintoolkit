...
C
      ...
      T(1) = 0.D0
      DO 1 I = 2,NN
        DS = (X(I)-X(I-1))**2 + (Y(I)-Y(I-1))**2 +       (Z(I)-Z(I-1))**
     +2
        IF (DS .EQ. 0.D0) GO TO 3
        T(I) = T(I-1) + DSQRT(DS)
    1 CONTINUE
...
C
    3 IER = I
     ...
     +     YP,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IER
      DOUBLE PRECISION    X(N), Y(N), W(N), P, D(N), SD(N), T11(N),     
...
C
      DO 1 I = 2,NM1
        ...
        S11I = -2.D0*S12I/DX
        R1 = PP*W(I)
        D11I = R1 - S11IM1 - S11I - (S11IM1*T11(IM1) -         S12IM1*T2
     +1(IM1))
        D12I = S12I - S12IM1 - (S11IM1*T12(IM1) - S12IM1*         T22(IM
     +1))
...
C
    ...
    2   CONTINUE
     ...
     +       U12,U21,U22, YS,YP,IER)
      ...
      INTEGER N, IER
      DOUBLE PRECISION    X(N), Y(N), W(N), P, D(N), SD(N), T11(N),     
     +   T12(N), T21(N), T22(N), U11(N), U12(N),        U21(N), U22(N), 
     +YS(N), YP(N)
C
C***********************************************************
C
...
C
      R1 = PP*W(NM1)
      D11I = R1 - S11I - S11NM1 + S11NM1*T11(1) -       S12NM1*T21(1) + 
     +S11I*T11(NM2) - S12I*T21(NM2)
      D12I = -S12NM1 - S12I + S11NM1*T12(1) - S12NM1*T22(1)       + S11I
     +*T12(NM2) - S12I*T22(NM2)
      D22I = DI + DNM1 + S12NM1*T12(1) + S22NM1*T22(1) +       S12I*T12(
     +NM2) + S22I*T22(NM2)
      DEN = D11I*D22I - D12I*D12I
...
C
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION ENDSLP (X1,X2,X3,Y1,Y2,Y3,SIGMA)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C
C Intrinsic functions called by ENDSLP:  ABS, AMAX1, AMIN1,
C                                          EXP
C
C***********************************************************
C
...
C
        T = DEXP(SIG1-SIGS)*((1.D0-DEXP(-SIG1))/                      (1
     +.D0-DEXP(-SIGS)))**2
      ENDIF
C
...
C
    1 T = ((Y3-Y1)*T-Y2+Y1)/(DXS*T-DX1)
      S1 = (Y2-Y1)/DX1
      IF (S1 .GE. 0.D0) THEN
        ENDSLP = DMIN1(DMAX1(0.D0,T), 3.D0*S1)
      ELSE
        ENDSLP = DMAX1(DMIN1(0.D0,T), 3.D0*S1)
      ENDIF
      RETURN
C
...
C
    2 ENDSLP = 0.D0
      RETURN
      END
      DOUBLE PRECISION FUNCTION HPPVAL (T,N,X,Y,YP,SIGMA, IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IER
...
C
        ...
        COSH2 = CM2 + 1.D0
        E = SIG*SM - CMM - CMM
        HPPVAL = SIG*((CM*SINH2-SM*COSH2)*(D1+D2) +              SIG*(CM
...
C
        ...
        SINH2 = SM2 + SB2
        E = SIG*SM - CMM - CMM
        HPVAL = S1 + ((CM*CM2-SM*SINH2)*(D1+D2) +                SIG*(CM
...
C
        ...
        ELSE
          ...
          TM = 1.D0 - EMS
          E = TM*(SIG*(1.D0+EMS) - TM - TM)
          HPVAL = S + (TM*((E2-E1)*(D1+D2) + TM*(D1-D2)) +            SI
     +G*((E1*EMS-E2)*D1 + (E1-E2*EMS)*D2))/E
        ENDIF
      ENDIF
      RETURN
C
...
C
    2 INTRVL = IL
      RETURN
      END
      DOUBLE PRECISION FUNCTION SIG0 (X1,X2,Y1,Y2,Y1P,Y2P,IFL,HBND,TOL, 
     -   IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER IFL, IER
...
C
C Intrinsic functions called by SIG0:  ABS, ALOG, AMAX1,
C                                        AMIN1, EXP, FLOAT,
C                                        SIGN, SQRT
C
C***********************************************************
...
C
      IF ((RF .LT. 0.D0  .AND.  DMIN1(Y1L,Y2L) .LT. BND)  .OR.    (RF .G
     +T. 0.D0  .AND.  BND .LT. DMAX1(Y1L,Y2L)))    GO TO 9
C
C Test for infinite tension required.
C
      S1 = Y1P
      S2 = Y2P
      IF ((Y1L .EQ. BND  .AND.  RF*S1 .GT. 0.D0)  .OR.    (Y2L .EQ. BND 
     + .AND.  RF*S2 .LT. 0.D0)) GO TO 7
C
...
C
      IF (S1*S2 .GE. 0.D0  .AND.  (T0*(S1+S2) .GE. 0.D0  .OR.    D0 .LT.
     + 0.D0)) GO TO 6
      IF (A0 .EQ. 0.D0) THEN
C
...
C   of F is negative.
C
      FMAX = DMAX1(1.D-3,DMIN1(DABS(Y1L-BND),DABS(Y2L-BND)))
      T = DMAX1(DABS(Y1L-BND),DABS(Y2L-BND))
      SIG = DX*DMAX1(DABS(S1),DABS(S2))/T
      DMAX = SIG*(1.D0-T/FMAX)
      SNEG = SIG - DMAX
      IF (LUN .GE. 0  .AND.  RF .LT. 0.D0) WRITE (LUN,100) F0,          
     +                         FMAX, SNEG
      IF (LUN .GE. 0  .AND.  RF .GT. 0.D0) WRITE (LUN,110) F0,          
     +                         FMAX, SNEG
  100 FORMAT (//1X,'SIG0 (LOWER BOUND) -- F(0) = ',E15.8,        ', FMAX
     + = ',E15.8/1X,46X,'SNEG = ',E15.8/)
  110 FORMAT (//1X,'SIG0 (UPPER BOUND) -- F(0) = ',E15.8,        ', FMAX
     + = ',E15.8/1X,46X,'SNEG = ',E15.8/)
      DSIG = SIG
      FNEG = FMAX
      D2 = S2 - S
...
C
      ...
      T = -B - DSIGN(T1,B)
      RSIG = 0.D0
      IF (RF*B .LT. 0.D0  .AND.  AA .NE. 0.D0  .AND.    T/AA .GT. 0.D0) 
     +RSIG = SIG + DLOG(T/AA)
      IF ((RF*B .GT. 0.D0  .OR.  AA .EQ. 0.D0)  .AND.    C/T .GT. 0.D0) 
     +RSIG = DLOG(C/T)
      IF ((RSIG .LE. 0.D0  .OR.  RSIG .GE. SIG)  .AND.    B .NE. 0.D0) G
     +O TO 3
C
...
C
  ...
  120 FORMAT (1X,3X,I2,' -- SIG = ',E15.8,', F = ',        E15.8)
...
C
      STOL = RTOL*SIG
      IF (DABS(DMAX) .LE. STOL  .OR.  (F .GE. 0.D0  .AND.    F .LE. FTOL
     +)  .OR.  DABS(F) .LE. RTOL) GO TO 6
C
C   Test for F0 = F = FMAX or F < 0 on the first iteration.
C
      IF (F0 .NE. F  .AND.  (NIT .GT. 1  .OR.  F .GT. 0.D0))   GO TO 5
C
...
C
  ...
  130 FORMAT (1X,8X,'DSIG = ',E15.8)
      IF (DABS(DSIG) .GT. DABS(DMAX)) GO TO 4
C
C   Restrict the step-size such that abs(DSIG) .GE. STOL/2.
C     Note that DSIG and DMAX have opposite signs.
C
      IF (DABS(DSIG) .LT. STOL/2.D0) DSIG = -DSIGN(STOL/2.D0,DMAX)
C
...
C
    9 IER = -2
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION SIG1 (X1,X2,Y1,Y2,Y1P,Y2P,IFL,HPBND,TOL,
     - IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER IFL, IER
...
C
C Intrinsic functions called by SIG1:  ABS, AMAX1, AMIN1,
C                                        EXP, FLOAT, SIGN,
C                                        SQRT
C
C***********************************************************
...
C Test for a valid constraint.
C
      IF ((RF .LT. 0.D0  .AND.  DMIN1(S1,S2,S) .LT. BND)  .OR.    (RF .G
     +T. 0.D0  .AND.  BND .LT. DMAX1(S1,S2,S)))    GO TO 8
C
C Test for infinite tension required.
C
      IF (S .EQ. BND  .AND.  (S1 .NE. S  .OR.  S2 .NE. S))   GO TO 6
C
...
C
      FMAX = (BND-S)*RF
      IF (LUN .GE. 0  .AND.  RF .LT. 0.D0) WRITE (LUN,100)              
     +                     F0, FMAX
      IF (LUN .GE. 0  .AND.  RF .GT. 0.D0) WRITE (LUN,110)              
     +                     F0, FMAX
  100 FORMAT (//1X,'SIG1 (LOWER BOUND) -- F(0) = ',E15.8,        ', FMAX
     + = ',E15.8/)
  110 FORMAT (//1X,'SIG1 (UPPER BOUND) -- F(0) = ',E15.8,        ', FMAX
     + = ',E15.8/)
      SIG = 2.D0 - A0/(3.D0*(BND-S))
      IF (STORE(SIG*DEXP(-SIG)+.5D0) .EQ. .5D0) GO TO 5
      DSIG = SIG
      DMAX = -2.D0*SIG
      FNEG = FMAX
      D1 = S - S1
...
C
        ...
        TM = 1.D0 - EMS
        DSINH = TM*(1.D0+EMS)
        SINHM = DSINH - SIG*EMS2
        COSHM = TM*TM
        C1 = SIG*COSHM*D2 - SINHM*D1PD2
        C2 = SIG*DSINH*D2 - COSHM*D1PD2
        A = EMS2*(SIG*TM*D2 + (TM-SIG)*D1PD2)
        E = SIG*DSINH - COSHM - COSHM
      ENDIF
C
...
C
        ...
        DMAX = DSIG
        FNEG = F0
        IF ( DABS(DSIG) .GT. DABS(T1)  .AND.       DABS(F) .LT. DABS(T2)
     +          ) THEN
C
          DSIG = T1
          F0 = T2
...
C
    4 DSIG = -F*DSIG/(F-F0)
      IF (LUN .GE. 0) WRITE (LUN,130) DSIG
  130 FORMAT (1X,8X,'DSIG = ',E15.8)
      IF (DABS(DSIG) .GT. DABS(DMAX)) GO TO 3
C
C   Restrict the step-size such that abs(DSIG) .GE. STOL/2.
C     Note that DSIG and DMAX have opposite signs.
C
...
C
    8 IER = -2
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION SIG2 (X1,X2,Y1,Y2,Y1P,Y2P,IFL,TOL, IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER IFL, IER
...
C
C Intrinsic functions called by SIG2:  ABS, AMAX1, AMIN1,
C                                        EXP, IABS, SQRT
C
C***********************************************************
C
...
C
      DX = X2 - X1
      IF (IABS(IFL) .NE. 1.D0  .OR.  DX .LE. 0.D0) GO TO 5
C
...
C
      ...
      D1 = S - Y1P
      D2 = Y2P - S
      IF ((IFL .GT. 0.D0  .AND.  DMIN1(D1,D2) .LT. 0.D0)  .OR.    (IFL .
     +LT. 0.D0  .AND.  DMAX1(D1,D2) .GT. 0.D0))    GO TO 6
C
C Test for infinite tension required.
C
...
C
  ...
  100 FORMAT (1X,'SIG2 -- SIG = ',E15.8,', F(SIG) = ',        E15.8/1X,2
     +9X,'FP(SIG) = ',E15.8)
      NIT = NIT + 1
C
C   Test for convergence.
C
      IF (FP .EQ. 0.D0) GO TO 3
      DSIG = -F/FP
      IF (DABS(DSIG) .LE. RTOL*SIG  .OR.  (F .GE. 0.D0  .AND.    F .LE. 
     +FTOL)  .OR.  DABS(F) .LE. RTOL) GO TO 3
C
C   Update SIG.
C
      SIG = SIG + DSIG
...
C
    6 IER = -2
     ...
     +    DSMAX,IER)
      ...
      DOUBLE PRECISION    X(N), Y(N), YP(N), TOL, B(5,N), BMAX,        S
     +IGMA(N), DSMAX
C
C***********************************************************
C
...
C                               STORE
C
C Intrinsic functions called by SIGBI:  ABS, AMAX1, AMIN1
C
C***********************************************************
C
      DATA SBIG/85.D0/
...
C
    5 DSMAX = DSM
      ...
      RETURN
      END
      SUBROUTINE SIGBP (N,X,Y,XP,YP,TOL,BL,BU,                  BMAX, SI
     +GMA, DSMAX,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IER
      DOUBLE PRECISION    X(N), Y(N), XP(N), YP(N), TOL, BL(N), BU(N),  
     +      BMAX, SIGMA(N), DSMAX
C
C***********************************************************
C
...
C
C Intrinsic functions called by SIGBP:  ABS, ALOG, AMAX1,
C                                         AMIN1, EXP, SIGN,
C                                         SQRT
C
C***********************************************************
...
C
  ...
  100   FORMAT (//1X,'SIGBP -- INTERVAL',I4,', BL = ',E10.3,          ',
     + BU = ',E10.3,', SIGIN = ',E15.8)
...
C
        T = DMIN1(BHI,-BLO)
        FMAX = DMAX1(1.D-3,T)
        SIG = DMAX1(DABS(V1),DABS(V2))/T
        DMAX = SIG*(1.D0-T/FMAX)
        SNEG = SIG - DMAX
        IF (LUN .GE. 0) WRITE (LUN,110) F0, FMAX, SNEG
  110   FORMAT (//1X,'F(0) = ',E15.8,', FMAX = ',E15.8,          ', SNEG
     + = ',E15.8/)
        DSIG = SIG
        FNEG = FMAX
        V2M1 = V2 - V1
...
C
          ...
          IF (V1 .GE. 0.D0) THEN
            DP = E*V1/(SIG*(DSQRT(EB*EB-E*E)+EB))
            DM = 0.D0
          ELSE
            DP = 0.D0
            DM = E*V1/(SIG*(DSQRT(EB*EB-E*E)+EB))
          ENDIF
        ELSE
C
...
C
          RSP = 0.D0
          IF (B .LT. 0.D0  .AND.  AA .NE. 0.D0  .AND.        T/AA .GT. 0
     +.D0) RSP = SIG + DLOG(T/AA)
          IF ((B .GT. 0.D0  .OR.  AA .EQ. 0.D0)  .AND.        C/T .GT. 0
     +.D0) RSP = DLOG(C/T)
          IF ((RSP .LE. 0.D0  .OR.  RSP .GE. SIG)  .AND.        B .NE. 0
     +.D0) THEN
C
C   The maximum is DP = 0 at the endpoints.
C
...
C
          RSM = 0.D0
          IF (B .GT. 0.D0  .AND.  AA .NE. 0.D0  .AND.        T/AA .GT. 0
     +.D0) RSM = SIG + DLOG(T/AA)
          IF ((B .LT. 0.D0  .OR.  AA .EQ. 0.D0)  .AND.        C/T .GT. 0
     +.D0) RSM = DLOG(C/T)
          IF ((RSM .LE. 0.D0  .OR.  RSM .GE. SIG)  .AND.        B .NE. 0
     +.D0) THEN
C
C   The minimum is DM = 0 at the endpoints.
C
...
C
        STOL = RTOL*SIG
        IF (DABS(DMAX) .LE. STOL  .OR.  (F .GE. 0.D0  .AND.      F .LE. 
     +FTOL)  .OR.  DABS(F) .LE. RTOL) GO TO 6
C
C   Test for F0 = F = FMAX or F < 0 on the first iteration.
C
        IF (F0 .NE. F  .AND.  (NIT .GT. 1  .OR.  F .GT. 0.D0))     GO TO
     + 5
C
C   F*F0 > 0 and either the new estimate would be outside of
C     the bracketing interval of length abs(DMAX) or F < 0
...
C
    9 DSMAX = DSM
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IER
      DOUBLE PRECISION    X(N), Y(N), YP(N), TOL, SIGMA(N), DSMAX
C
C***********************************************************
C
C                                                From TSPACK
...
C
        SIG = SBIG
        IF ((D1D2 .EQ. 0.D0  .AND.  S1 .NE. S2)  .OR.      (S .EQ. 0.D0 
     + .AND.  S1*S2 .GT. 0.D0)) GO TO 7
C
...
C
  ...
  110   FORMAT (5X,'CONVEXITY -- SIG = ',E15.8,          ', F(SIG) = ',E
     +15.8/1X,35X,'FP(SIG) = ',          E15.8)
        NIT = NIT + 1
C
C   Test for convergence.
C
        IF (FP .EQ. 0.D0) GO TO 7
        DSIG = -F/FP
        IF (DABS(DSIG) .LE. RTOL*SIG  .OR.  (F .GE. 0.D0  .AND.      F .
     +LE. FTOL)  .OR.  DABS(F) .LE. RTOL) GO TO 7
C
C   Update SIG.
C
        SIG = SIG + DSIG
...
C     Note that DSIG and DMAX have opposite signs.
C
        IF (DABS(DSIG) .LT. STOL/2.D0) DSIG = -DSIGN(STOL/2.D0,         
     +                     DMAX)
C
...
C
  ...
  130   FORMAT (1X,10X,I2,' -- SIG = ',E15.8,', F = ',          E15.8)
C
C   Test for convergence.
C
        STOL = RTOL*SIG
        IF ( DABS(DMAX) .LE. STOL  .OR.  (F .GE. 0.D0  .AND.      F .LE.
     + FTOL)  .OR.  DABS(F) .LE. RTOL ) GO TO 7
        DMAX = DMAX + DSIG
        IF ( F0*F .GT. 0.D0  .AND.  DABS(F) .GE. DABS(F0) )     GO TO 6
...
C
          ...
          DMAX = DSIG
          FNEG = F0
          IF ( DABS(DSIG) .GT. DABS(T1)  .AND.         DABS(F) .LT. DABS
     +(T2)         ) THEN
C
            DSIG = T1
            F0 = T2
...
C
   10 DSMAX = DSM
     ...
     +WK, YS,YP,IER)
      ...
      LOGICAL PERIOD
      INTEGER N, IER
      DOUBLE PRECISION    X(N), Y(N), SIGMA(N), W(N), SM, SMTOL,        
     +WK(N,10), YS(N), YP(N)
C
C***********************************************************
C
...
C
      IER = -1
      IF (NN .LT. 2  .OR.  (NN .LT. 3  .AND.  PER)  .OR.    SM .LE. 0.D0
     +  .OR.  SMTOL .LE. 0.D0  .OR.    SMTOL .GE. 1.D0) RETURN
      C11 = 0.D0
      C12 = 0.D0
      C22 = 0.D0
      R1 = 0.D0
...
C
  ...
  100   FORMAT (///1H ,'SMCRV -- THE CONSTRAINT IS NOT ',          'ACTI
     +VE AND THE FIT IS LINEAR.'/)
        RETURN
      ENDIF
C
...
C
  ...
  110 FORMAT (///1H ,3X,'SMCRV -- SM = ',E10.4,', SMTOL = ',        E14.
     +8,', G(0) = ',E15.8///)
C
...
C
    4 IF (.NOT. PER) THEN
     ...
     +  WK(1,5),WK(1,6),WK(1,7),WK(1,8),               WK(1,9),WK(1,10),
     + YS,YP,IERR)
      ENDIF
      Q2 = 0.D0
      DO 5 I = 1,NN
  ...
  120   FORMAT (/1X,I2,' -- P = ',E15.8,',  G = ',E15.8/          6X,'P0
     + = ',E15.8,', G0 = ',E15.8)
      ENDIF
C
C   Test for convergence.
...
C***********************************************************
C
      DATA C1/.1666666666659D0/,     C2/.8333333431546D-2/,     C3/.1984
     +107350948D-3/,     C4/.2768286868175D-5/
      AX = DABS(X)
      XS = AX*AX
      IF (AX .LE. .5D0) THEN
C
...
C
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION TSINTL (A,B,N,X,Y,YP,SIGMA, IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IER
...
C
     ...
     +(D1+D2) + SIG*(CM*SM1-(SM+SIG)*CMM1)*D2)/             ((SIG/DX)**2
     +*E)
      ELSE
C
C   SIG > .5.
C
     ...
     +     TM*SB1)*D2 - (TM*(TM*T-E1+E2-TP*SB1) +                SIG*(E1
     +*EMS-E2+2.D0*SB1*EMS))*(D1+D2))/                ((SIG/DX)**2*E)
        ENDIF
      ENDIF
C
...
C   SIG = 0.
C
          SUM = SUM + DX*((Y(I)+Y(IP1))/2.D0 -                    DX*(YP
     +(IP1)-YP(I))/12.D0)
        ELSEIF (SIG .LE. .5D0) THEN
C
C   0 .LT. SIG .LE. .5.
...
C
     ...
     +   (1.D0-EMS)-2.D0)*(YP(IP1)-YP(I))/                (SIG*SIG))/2.D
     +0
        ENDIF
    3   CONTINUE
C
...
C   SIG = 0.
C
        SUM = SUM + U*(Y1 + U*(6.D0*S1 + B2*(4.D0*D1 +                (4
     +.D0-3.D0*B2)*(D1-D2)))/12.D0)
      ELSEIF (SIG .LE. .5D0) THEN
C
C   0 .LT. SIG .LE. .5.
C
     ...
     + (D1+D2) + SIG*(CM*SM2-(SM+SIG)*CMM2)*D1)              /((SIG/DX)*
     +*2*E)
      ELSE
C
C   SIG > .5.
C
     ...
     +     TM*SB2)*D1 - (TM*(TM*T-E2+E1-TP*SB2) +                SIG*(E2
     +*EMS-E1+2.D0*SB2*EMS))*(D1+D2))/                ((SIG/DX)**2*E)
        ENDIF
      ENDIF
      GO TO 6
C
...
C   SIG = 0.
C
        SUM = SUM - U*(Y2 - U*(6.D0*S2 - B1*(4.D0*D2 +              (3.D
     +0*B1-4.D0)*(D1-D2)))/12.D0)
      ELSEIF (SIG .LE. .5D0) THEN
C
C   0 .LT. SIG .LE. .5.
C
        ...
        E = SIG*SM - CMM - CMM
        SUM = SUM - U*(Y2 - S2*U/2.D0) - ((CM*CMM1-SM*SM1)*             
     + (D1+D2) + SIG*(CM*SM1-(SM+SIG)*CMM1)*D2)              /((SIG/DX)*
     +*2*E)
      ELSE
C
C   SIG > .5.
...
C
    8 IER = -2
     ...
     +     YP, SIGMA,ICFLG,IER)
      ...
      LOGICAL PER
      DOUBLE PRECISION    X(N), Y(N), B(5,N), BMAX, WK(LWK), YP(N),     
     +   SIGMA(N)
C
C***********************************************************
C
...
C                               YPC2P
C
C Intrinsic functions called by TSPBI:  ABS, AMAX1
C
C***********************************************************
C
      LOGICAL LOOP2
...
C
      IF (NN .LT. 2  .OR.  (PER  .AND.  NN .LT. 3)  .OR.    NCD .LT. 1  
     +.OR.  NCD .GT. 2) GO TO 11
      IF ( NCD .EQ. 2  .AND.  (LWK .LT. 2*NM1  .OR.     (PER  .AND.  LWK
     + .LT. 3*NM1)) ) GO TO 12
C
...
C
        CALL SIGBI (NN,X,Y,YP,STOL,B,BMAX, SIGMA, ICFLG,              DS
     +MAX,IERR)
        GO TO 10
      ENDIF
C
...
C     invalid.
C
        CALL YPC2 (NN,X,Y,SIGMA,IENDC,IENDC,YP(1),YP(NN),             WK
     +, YP,IERR)
...
C
    ...
    3     CONTINUE
     ...
     +MAX,ICNT)
        IF (.NOT. PER) THEN
          CALL YPC2 (NN,X,Y,SIGMA,IENDC,IENDC,YP(1),YP(NN),             
     +  WK(NN), YP,IERR)
        ELSE
...
C
   14 IER = -4
     ...
     + LWK, WK, T,XP,YP,SIGMA,IER)
      ...
      LOGICAL PER
      DOUBLE PRECISION    X(N), Y(N), BL(N), BU(N), BMAX, WK(LWK), T(N),
     +        XP(N), YP(N), SIGMA(N)
C
C***********************************************************
C
...
C                               YPC2, YPC2P
C
C Intrinsic functions called by TSPBP:  ABS, AMAX1
C
C***********************************************************
C
      LOGICAL LOOP2
...
C
      ...
      IF (NN .LT. 2  .OR.  (PER  .AND.  NN .LT. 3)  .OR.    NCD .LT. 1  
     +.OR.  NCD .GT. 2) GO TO 11
      IF ( NCD .EQ. 2  .AND.  (LWK .LT. 3*NM1  .OR.     (PER  .AND.  LWK
     + .LT. 4*NM1)) ) GO TO 12
C
...
C   Compute tension factors.
C
        CALL SIGBP (NN,X,Y,XP,YP,STOL,BL,BU,              BMAX, SIGMA, D
     +SMAX,IERR)
...
C
        CALL YPC2 (NN,T,X,SIGMA,IENDC,IENDC,XP(1),XP(NN),             WK
     +, XP,IERR)
        CALL YPC2 (NN,T,Y,SIGMA,IENDC,IENDC,YP(1),YP(NN),             WK
     +, YP,IERR)
        IF (IERR .EQ. 1) GO TO 11
...
C
    ...
    3     CONTINUE
     ...
     +SMAX,ICNT)
        ...
        IF (.NOT. PER) THEN
          CALL YPC2 (NN,T,X,SIGMA,IENDC,IENDC,XP(1),XP(NN),             
     +  WK(N2M1), XP,IERR)
          CALL YPC2 (NN,T,Y,SIGMA,IENDC,IENDC,YP(1),YP(NN),             
     +  WK(N2M1), YP,IERR)
        ELSE
        ...
        DO 4 I = 2,NM1
          EX = DABS(XP(I)-WK(I))
          IF (WK(I) .NE. 0.D0) EX = EX/DABS(WK(I))
          EY = DABS(YP(I)-WK(NM1+I))
          IF (WK(NM1+I) .NE. 0.D0) EY = EY/DABS(WK(NM1+I))
          DYP = DMAX1(DYP,EX,EY)
    4     CONTINUE
        IF (ICNT .EQ. 0  .OR.  DYP .LE. DYPTOL) GO TO 7
        IF (.NOT. LOOP2) THEN
...
C                               YPC2P
C
C Intrinsic functions called by TSPSI:  ABS, AMAX1
C
C***********************************************************
C
      DATA SBIG/85.D0/
...
C
     ...
     +.OR.  NCD .GT. 2) GO TO 11
      IF (UNIFRM) THEN
        IF ( NCD .EQ. 2  .AND.  (LWK .LT. NM1  .OR.       (PER  .AND.  L
     +WK .LT. 2*NM1)) ) GO TO 12
        SIG = SIGMA(1)
        IF (SIG .LT. 0.D0  .OR.  SIG .GT. SBIG) GO TO 13
      ELSE
        IF ( NCD .EQ. 2  .AND.  (LWK .LT. 2*NM1  .OR.       (PER  .AND. 
     + LWK .LT. 3*NM1)) ) GO TO 12
        SIG = 0.D0
      ENDIF
C
...
C
   14 IER = -4
     ...
     +  LWK, WK, T,XP,YP,ZP,SIGMA,IER)
      ...
      LOGICAL PER, UNIFRM
      DOUBLE PRECISION    X(N), Y(N), Z(N), WK(LWK), T(N), XP(N), YP(N),
     +        ZP(N), SIGMA(N)
C
C***********************************************************
C
...
C                               YPC1P, YPC2, YPC2P
C
C Intrinsic functions called by TSPSP:  ABS, AMAX1
C
C***********************************************************
C
      LOGICAL SCURV
...
C
     ...
     +WK .LT. N2M2)) ) GO TO 12
        ...
        IF (SIG .LT. 0.D0  .OR.  SIG .GT. SBIG) GO TO 13
      ELSE
        IF ( NCD .EQ. 2  .AND.  (LWK .LT. (ND+1)*NM1  .OR.       (PER  .
     +AND.  LWK .LT. (ND+2)*NM1)) ) GO TO 12
        SIG = 0.D0
      ENDIF
C
C Compute the sequence of parameter values T.
    ...
    3       CONTINUE
     ...
     +             IERR)
        ...
        DO 4 I = 2,NM1
          ...
          IF (WK(NM1+I) .NE. 0.D0) EY = EY/DABS(WK(NM1+I))
          IF (SCURV) THEN
            EZ = DABS(ZP(I)-WK(N2M2+I))
            IF (WK(N2M2+I) .NE. 0.D0) EZ = EZ/DABS(WK(N2M2+I))
          ENDIF
          DYP = DMAX1(DYP,EX,EY,EZ)
    4     CONTINUE
...
C
   16 IER = -6
     ...
     +      SIGMA,YS,YP, NIT,IER)
      ...
      LOGICAL PER, UNIFRM
      DOUBLE PRECISION    X(N), Y(N), W(N), SM, SMTOL, WK(LWK), SIGMA(N)
     +,        YS(N), YP(N)
C
C***********************************************************
C
...
C
      ...
      IF (UNIFRM) THEN
        IF ( LWK .LT. 6*NN  .OR.       (PER  .AND.  LWK .LT. 10*NN) ) GO
     + TO 12
        SIG = SIGMA(1)
        IF (SIG .LT. 0.D0  .OR.  SIG .GT. SBIG) GO TO 13
      ELSE
        IF ( LWK .LT. 7*NN  .OR.       (PER  .AND.  LWK .LT. 11*NN) ) GO
     + TO 12
        SIG = 0.D0
      ENDIF
C
...
C
   13 IER = -3
     ...
     +  IER)
      ...
      DOUBLE PRECISION    X(N), Y(N), YP(N), SIGMA(N), TE(NE), V(NE)
C
C***********************************************************
C
C                                                From TSPACK
...
C
      IF (N .LT. 2  .OR.  IFLG .LT. 0  .OR.  IFLG .GT. 2    .OR.  NVAL .
     +LT. 1) GO TO 2
C
...
C
    3 IER = -2
     ...
     + TE, VX,VY,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IFLAG, NE, IER
      DOUBLE PRECISION    T(N), X(N), Y(N), XP(N), YP(N), SIGMA(N),     
     +   TE(NE), VX(NE), VY(NE)
C
C***********************************************************
C
...
C
    3 IER = -2
     ...
     +      TE, VX,VY,VZ,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, IFLAG, NE, IER
      DOUBLE PRECISION    T(N), X(N), Y(N), Z(N), XP(N), YP(N), ZP(N),  
     +      SIGMA(N), TE(NE), VX(NE), VY(NE), VZ(NE)
C
C***********************************************************
C
C                                                From TSPACK
...
C
C Intrinsic functions called by YPC1:  ABS, AMAX1, AMIN1,
C                                        SIGN
C
C***********************************************************
C
...
C
      ...
      IF (SI .GE. 0.D0) THEN
        YP(1) = DMIN1(DMAX1(0.D0,T), 3.D0*SI)
      ELSE
        YP(1) = DMAX1(DMIN1(0.D0,T), 3.D0*SI)
      ENDIF
C
...
C
      DO 1 I = 2,NM1
        ...
        ASI = DABS(SI)
        SGN = DSIGN(1.D0,SI)
        IF (ASIM1 .GT. ASI) SGN = DSIGN(1.D0,SIM1)
        IF (SGN .GT. 0.D0) THEN
          YP(I) = DMIN1(DMAX1(0.D0,T), 3.D0*DMIN1(ASIM1,ASI))
        ELSE
          YP(I) = DMAX1(DMIN1(0.D0,T), -3.D0*DMIN1(ASIM1,ASI))
        ENDIF
    1   CONTINUE
C
...
C
      ...
      IF (SI .GE. 0.D0) THEN
        YP(N) = DMIN1(DMAX1(0.D0,T), 3.D0*SI)
      ELSE
        YP(N) = DMAX1(DMIN1(0.D0,T), 3.D0*SI)
      ENDIF
      IER = 0
      RETURN
C
...
C
C Intrinsic functions called by YPC1P:  ABS, AMAX1, AMIN1,
C                                         SIGN
C
C***********************************************************
C
...
C
      ...
      IF (ASIM1 .GT. ASI) SGN = DSIGN(1.D0,SIM1)
      IF (SGN .GT. 0.D0) THEN
        YP(1) = DMIN1(DMAX1(0.D0,T), 3.D0*DMIN1(ASIM1,ASI))
      ELSE
        YP(1) = DMAX1(DMIN1(0.D0,T), -3.D0*DMIN1(ASIM1,ASI))
      ENDIF
      YP(N) = YP(1)
C
C No error encountered.
...
C
    3 IER = I + 1
      RETURN
      END
      SUBROUTINE YPC2 (N,X,Y,SIGMA,ISL1,ISLN,BV1,BVN,                 WK
     +, YP,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER N, ISL1, ISLN, IER
      DOUBLE PRECISION    X(N), Y(N), SIGMA(N), BV1, BVN, WK(N), YP(N)
C
C***********************************************************
C
C                                                From TSPACK
...
C
      NN = N
      IF (NN .LT. 2  .OR.  ISL1 .LT. 0  .OR.  ISL1 .GT. 3    .OR.  ISLN 
     +.LT. 0  .OR.  ISLN .GT. 3) GO TO 3
      NM1 = NN - 1
C
...
C
      YPNM1 = (R1 + RNM1 - SDNM1*YP(1) - SD1*YP(NM2))/        (D1 + DNM1
     + + SDNM1*WK(1) + SD1*WK(NM2))
C
C Back substitute for the remainder of the solution
C   components.
C
