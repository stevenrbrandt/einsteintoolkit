...
use affine, only:  theta, sint, cost
implicit none
...
allocate(theta(n_x), iperm(n_x), rank(n_x), cost(n_x), sint(n_x))
...
if (model_switch < 2) then
   Do ns = 1, 2            ! first North - then South
      Do i = 1, nn
         Do i1 = 1, nn
            if(ns == 1) then ! North
               cost(nn*(i-1)+i1)      = (2.0_wp - pp(i,i1))/pp(i,i1)
               theta    (nn*(i-1)+i1) = acos(cost(nn*(i-1)+i1))
            else             ! South
               cost(nn*(i-1)+i1 + nn*nn)  = (pp(i,i1) - 2.0_wp)/pp(i,i1)
               theta(nn*(i-1)+i1 + nn*nn) = acos(cost(nn*(i-1)+i1 + nn*nn))
            endif
         End Do
      End Do
   End Do
   ...
   !           theta array that is in the Ith location in the sorted order.


   ! rank(i) = the index of the value in the sorted order of the
   !           theta array that is in the Ith location in the original order.
   Do i = 1, n_x   
      Do i1 = 1, n_x
         if ( iperm(i1) == i ) rank(i) = i1
      End Do
   End Do
   ! in other words, iperm and rank are defined as
   !  theta(i) = theta_orig(iperm(i))
   !
   !  theta_orig(i) = theta(rank(i))

   ! next: we have to sort cost as well!
   ! use array sint as temporary for cost
   sint = cost
   Do i = 1, n_x   
        cost(i) = sint(iperm(i))
   End Do

   ! taking the sin is easy now, it will be sorted automatically
   ...
   if(equidist_theta) then
      ...
      cost(nn+1) = 0.0_wp
      do i = 1, nn
         cost(i)      = -h * dble(nn - i + 1)
         cost(nn+1+i) =  h * dble(i)
         theta(i) = acos(cost(i))
      enddo
      cost(1)   = -1.0_wp
      cost(n_x) =  1.0_wp     
...
if (debug > 0) then
 print *, 'values of cos(th) written to file cos_theta_input.dat'
   open(86, file = 'cos_theta_input.dat')
   do i = 1, n_x
      write(86,*) i, cost(i)
   enddo
   close(86)
...
use null_grid, only : q=>qs, p=>ps
use affine, only: sigma, p_omega, u_zero
use Model
...
if (debug) write(88,99) time, uhatofu(middle_nn)
...
!!!!!->  phase = cmplx(q(i,i1), p(i,i1))/cmplx(q(i,i1), -p(i,i1))
         ...
         ! But we are inconsistent, see Phys. Rev. D 56, 6298 (1997), 
         ! Eq. (44), which says: N_{North}(q,p) = e^{-2 i \phi) N(theta,phi),
         ! (the same transformation rule applies to J spin weight is all that
         ! matters). 
         ! In the paper, Eq. (43) is overkill, since 
         ! z_N = tan(\theta/2) e^{i \phi}, z_S=1/z_N, hence
         ! N(q,p) = \bar z / z N(theta,phi) holds in both patches.
         ! correction : N(q,p) = z/\bar z N(theta,phi)
         phase = cmplx(q(i,i1), p(i,i1))/cmplx(q(i,i1), -p(i,i1))
      endif
...
use affine, only: sigma, p_omega, udot, omega_dot
use Model
...
Do i = 1, nn
   Do i1 = 1, nn
      ...
      Rdot(i,i1,2)      = (rhatS * omdotS + rhat_dotS * omvec(indexS)) &
...
subroutine Spheroid_RRdotCgamma(Rout, Rdotout, Cgammaout, &
...
gugg = flipper*eps/sqrt(1.0_wp + eps * cost**2)**3      ! -sigma/sin2theta
