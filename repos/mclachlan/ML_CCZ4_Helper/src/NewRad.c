...
void ML_CCZ4_NewRad(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_CCZ4_NewRad);
  ...
  if (!have_Theta_index) {
    Theta_index = CCTK_VarIndex("ML_CCZ4::Theta");
    Thetarhs_index = CCTK_VarIndex("ML_CCZ4::Thetarhs");
  ...
  if (!have_A_index) {
    ...
    Arhs_index = CCTK_VarIndex("ML_CCZ4::Arhs");
    have_A_index = true;
  }
  if (A_index >= 0) {
    if(CCTK_IsFunctionAliased("Driver_RequireValidData")) {
  ...
  if (!have_B_index) {
    ...
    Brhs_index = CCTK_VarIndex("ML_CCZ4::B1rhs");
    have_B_index = true;
  }
  if (B_index >= 0) {
    if(CCTK_IsFunctionAliased("Driver_RequireValidData")) {
