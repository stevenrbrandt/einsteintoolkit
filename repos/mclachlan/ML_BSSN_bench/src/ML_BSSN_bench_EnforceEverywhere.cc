...
static void ML_BSSN_bench_EnforceEverywhere_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(ML_BSSN_bench_EnforceEverywhere,
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_bench_EnforceEverywhere);
}
extern "C" void ML_BSSN_bench_EnforceEverywhere(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_ML_BSSN_bench_EnforceEverywhere
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_BSSN_bench_EnforceEverywhere);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_bench_EnforceEverywhere_Body");
  }
  if (cctk_iteration % ML_BSSN_bench_EnforceEverywhere_calc_every != ML_BSSN_bench_EnforceEverywhere_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "ML_BSSN_bench_EnforceEverywhere", 3, groups);
  ...
  LoopOverEverything(cctkGH, ML_BSSN_bench_EnforceEverywhere_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_bench_EnforceEverywhere_Body");
  }
}
