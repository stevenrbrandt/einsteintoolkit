...
void ML_BSSN_ND_NewRad(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_BSSN_ND_NewRad);
  ...
  if (!have_Theta_index) {
    Theta_index = CCTK_VarIndex("ML_BSSN_ND::Theta");
    Thetarhs_index = CCTK_VarIndex("ML_BSSN_ND::Thetarhs");
  ...
  if (!have_A_index) {
    A_index = CCTK_VarIndex("ML_BSSN_ND::A");
    Arhs_index = CCTK_VarIndex("ML_BSSN_ND::Arhs");
    have_A_index = true;
  }
  if (A_index >= 0) {
    if(CCTK_IsFunctionAliased("Driver_RequireValidData")) {
  ...
  if (!have_B_index) {
    B_index = CCTK_VarIndex("ML_BSSN_ND::B1");
    Brhs_index = CCTK_VarIndex("ML_BSSN_ND::B1rhs");
    have_B_index = true;
  }
  if (B_index >= 0) {
    if(CCTK_IsFunctionAliased("Driver_RequireValidData")) {
