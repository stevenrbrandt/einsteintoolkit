...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_RHS_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_RHS_SelectBCs);
  #else
...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_RHS
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_RHS);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    ...
    "ML_WaveToy::WT_u",
    "ML_WaveToy::WT_urhs"};
  AssertGroupStorage(cctkGH, "WT_RHS", 4, groups);
  
  EnsureStencilFits(cctkGH, "WT_RHS", 2, 2, 2);
  
  LoopOverInterior(cctkGH, WT_RHS_Body);
  if (verbose > 1)
  {
