...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_EnergyBoundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_EnergyBoundary_SelectBCs);
  #else
...
static void WT_EnergyBoundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(WT_EnergyBoundary,
  ...
  CCTK_ENDLOOP3STR(WT_EnergyBoundary);
}
extern "C" void WT_EnergyBoundary(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_EnergyBoundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_EnergyBoundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    "ML_WaveToy::WT_eps"};
  AssertGroupStorage(cctkGH, "WT_EnergyBoundary", 1, groups);
  
  
  LoopOverBoundary(cctkGH, WT_EnergyBoundary_Body);
  if (verbose > 1)
  {
