...
static void CL_BSSN_convertFromADMBase_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(CL_BSSN_convertFromADMBase,
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    trKL = 
      kmadd(kxxL,gu11,kmadd(kyyL,gu22,kmadd(ToReal(2),kmadd(kxyL,gu21,kmadd(kxzL,gu31,kmul(kyzL,gu32))),kmul(kzzL,gu33))));
  ...
  CCTK_ENDLOOP3STR(CL_BSSN_convertFromADMBase);
}
extern "C" void CL_BSSN_convertFromADMBase(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CL_BSSN_convertFromADMBase
  DECLARE_CCTK_ARGUMENTS_CHECKED(CL_BSSN_convertFromADMBase);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CL_BSSN_convertFromADMBase_Body");
  }
  if (cctk_iteration % CL_BSSN_convertFromADMBase_calc_every != CL_BSSN_convertFromADMBase_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "CL_BSSN_convertFromADMBase", 10, groups);
  ...
  LoopOverEverything(cctkGH, CL_BSSN_convertFromADMBase_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CL_BSSN_convertFromADMBase_Body");
  }
}
