...
extern "C" void CL_BSSN_convertToADMBaseDtLapseShiftBoundary_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CL_BSSN_convertToADMBaseDtLapseShiftBoundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(CL_BSSN_convertToADMBaseDtLapseShiftBoundary_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % CL_BSSN_convertToADMBaseDtLapseShiftBoundary_calc_every != CL_BSSN_convertToADMBaseDtLapseShiftBoundary_calc_offset)
    return;
...
static void CL_BSSN_convertToADMBaseDtLapseShiftBoundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(CL_BSSN_convertToADMBaseDtLapseShiftBoundary,
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC dtalpL CCTK_ATTRIBUTE_UNUSED = 
      kneg(kmul(kmul(trKL,kpow(alphaL,harmonicN)),ToReal(harmonicF)));
    
    CCTK_REAL_VEC dtbetaxL CCTK_ATTRIBUTE_UNUSED = 
      kmul(B1L,ToReal(ShiftGammaCoeff));
    
    CCTK_REAL_VEC dtbetayL CCTK_ATTRIBUTE_UNUSED = 
      kmul(B2L,ToReal(ShiftGammaCoeff));
    
    CCTK_REAL_VEC dtbetazL CCTK_ATTRIBUTE_UNUSED = 
      kmul(B3L,ToReal(ShiftGammaCoeff));
    /* Copy local copies back to grid functions */
  ...
  CCTK_ENDLOOP3STR(CL_BSSN_convertToADMBaseDtLapseShiftBoundary);
}
extern "C" void CL_BSSN_convertToADMBaseDtLapseShiftBoundary(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CL_BSSN_convertToADMBaseDtLapseShiftBoundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(CL_BSSN_convertToADMBaseDtLapseShiftBoundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CL_BSSN_convertToADMBaseDtLapseShiftBoundary_Body");
  }
  if (cctk_iteration % CL_BSSN_convertToADMBaseDtLapseShiftBoundary_calc_every != CL_BSSN_convertToADMBaseDtLapseShiftBoundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "CL_BSSN_convertToADMBaseDtLapseShiftBoundary", 5, groups);
  ...
  LoopOverBoundaryWithGhosts(cctkGH, CL_BSSN_convertToADMBaseDtLapseShiftBoundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CL_BSSN_convertToADMBaseDtLapseShiftBoundary_Body");
  }
}
