...
extern "C" void CL_BSSN_boundary_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CL_BSSN_boundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(CL_BSSN_boundary_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % CL_BSSN_boundary_calc_every != CL_BSSN_boundary_calc_offset)
    return;
...
static void CL_BSSN_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(CL_BSSN_boundary,
  ...
  CCTK_ENDLOOP3STR(CL_BSSN_boundary);
}
extern "C" void CL_BSSN_boundary(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_CL_BSSN_boundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(CL_BSSN_boundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CL_BSSN_boundary_Body");
  }
  if (cctk_iteration % CL_BSSN_boundary_calc_every != CL_BSSN_boundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "CL_BSSN_boundary", 12, groups);
  ...
  LoopOverBoundaryWithGhosts(cctkGH, CL_BSSN_boundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CL_BSSN_boundary_Body");
  }
}
