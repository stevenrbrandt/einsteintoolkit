...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_ML_ADM_boundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_ADM_boundary_SelectBCs);
  #else
  ...
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_ADM::ML_curv","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for ML_ADM::ML_curv.");
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_ADM::ML_lapse","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for ML_ADM::ML_lapse.");
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_ADM::ML_metric","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for ML_ADM::ML_metric.");
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_ADM::ML_shift","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for ML_ADM::ML_shift.");
  return;
}

static void ML_ADM_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_ENDLOOP3STR(ML_ADM_boundary);
}
extern "C" void ML_ADM_boundary(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_ML_ADM_boundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_ADM_boundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    ...
    "ML_ADM::ML_metric",
    "ML_ADM::ML_shift"};
  AssertGroupStorage(cctkGH, "ML_ADM_boundary", 4, groups);
  ...
  LoopOverBoundaryWithGhosts(cctkGH, ML_ADM_boundary_Body);
  if (verbose > 1)
  {
