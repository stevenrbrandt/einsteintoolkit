...
static void ML_ADM_convertFromADMBase_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_ENDLOOP3STR(ML_ADM_convertFromADMBase);
}
extern "C" void ML_ADM_convertFromADMBase(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_ML_ADM_convertFromADMBase
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_ADM_convertFromADMBase);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    ...
    "ML_ADM::ML_metric",
    "ML_ADM::ML_shift"};
  AssertGroupStorage(cctkGH, "ML_ADM_convertFromADMBase", 8, groups);
  ...
  LoopOverEverything(cctkGH, ML_ADM_convertFromADMBase_Body);
  if (verbose > 1)
  {
