...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_CL_RHS_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_CL_RHS_SelectBCs);
  #else
...
static void WT_CL_RHS_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_CL_RHS
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_CL_RHS);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  const char* const groups[] = {
    ...
    "ML_WaveToy_CL::WT_u",
    "ML_WaveToy_CL::WT_urhs"};
  AssertGroupStorage(cctkGH, "WT_CL_RHS", 4, groups);
  
  EnsureStencilFits(cctkGH, "WT_CL_RHS", 2, 2, 2);
  
  LoopOverInterior(cctkGH, WT_CL_RHS_Body);
  if (verbose > 1)
  {
