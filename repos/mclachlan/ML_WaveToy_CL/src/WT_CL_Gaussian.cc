...
static void WT_CL_Gaussian_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  "    kmul(kexp(kmul(ToReal(pow(width,-2)),kmul(ToReal(-0.5),kmul(rL,rL)))),ToReal(amplitude));\n"
  "  \n"
  ...
  "}\n"
  "CCTK_ENDLOOP3STR(WT_CL_Gaussian);\n"
  ""
  ;
  ...
  OpenCLRunTime_CallKernel(cctkGH, CCTK_THORNSTRING, "WT_CL_Gaussian",
                           sources, groups, NULL, NULL, NULL, -1,
                           imin, imax, &kernel);
  
}
extern "C" void WT_CL_Gaussian(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_WT_CL_Gaussian
  DECLARE_CCTK_ARGUMENTS_CHECKED(WT_CL_Gaussian);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering WT_CL_Gaussian_Body");
  }
  if (cctk_iteration % WT_CL_Gaussian_calc_every != WT_CL_Gaussian_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "ML_WaveToy_CL::WT_rho",
    "ML_WaveToy_CL::WT_u"};
  AssertGroupStorage(cctkGH, "WT_CL_Gaussian", 3, groups);
  
  
  LoopOverEverything(cctkGH, WT_CL_Gaussian_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving WT_CL_Gaussian_Body");
  }
}
