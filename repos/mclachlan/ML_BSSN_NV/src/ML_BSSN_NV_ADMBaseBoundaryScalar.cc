...
extern "C" void ML_BSSN_NV_ADMBaseBoundaryScalar_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_ML_BSSN_NV_ADMBaseBoundaryScalar_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_BSSN_NV_ADMBaseBoundaryScalar_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ML_BSSN_NV_ADMBaseBoundaryScalar_calc_every != ML_BSSN_NV_ADMBaseBoundaryScalar_calc_offset)
    return;
...
static void ML_BSSN_NV_ADMBaseBoundaryScalar_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ML_BSSN_NV_ADMBaseBoundaryScalar,
  ...
  CCTK_ENDLOOP3(ML_BSSN_NV_ADMBaseBoundaryScalar);
}
extern "C" void ML_BSSN_NV_ADMBaseBoundaryScalar(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_ML_BSSN_NV_ADMBaseBoundaryScalar
  DECLARE_CCTK_ARGUMENTS_CHECKED(ML_BSSN_NV_ADMBaseBoundaryScalar);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_NV_ADMBaseBoundaryScalar_Body");
  }
  if (cctk_iteration % ML_BSSN_NV_ADMBaseBoundaryScalar_calc_every != ML_BSSN_NV_ADMBaseBoundaryScalar_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    "ADMBase::dtlapse",
    "ADMBase::dtshift"};
  AssertGroupStorage(cctkGH, "ML_BSSN_NV_ADMBaseBoundaryScalar", 2, groups);
  ...
  LoopOverBoundary(cctkGH, ML_BSSN_NV_ADMBaseBoundaryScalar_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_NV_ADMBaseBoundaryScalar_Body");
  }
}
