                              ...
                              CCTK_INT init_coeff)
{
  ...
  ENTER_LEVEL_MODE(cctkGH,level) {
    ...
    std::vector<CCTK_REAL> zero(number_of_equations);
...
//    if (init_coeff) CT_InitializeCoefficients(CCTK_PASS_CTOC);
    ...
    while (norm > tol && step < nsteps)
    {
      ...
      if (rset_psi) CT_Reset(CCTK_PASS_CTOC, "CT_MultiLevel::ct_err", &zero[0], 0);
...
//      CT_InitializeCoefficients(CCTK_PASS_CTOC);
      ...
      CT_CalcPsiResidual(CCTK_PASS_CTOC, step, 0, &tmp, 1);
