...
#pragma omp parallel reduction(+: sum, n)
        ...
        CCTK_INT localn = n, globaln;

        MPI_Reduce(&localsum, &globalsum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(&localn, &globaln, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Bcast(&globalsum, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&globaln, 1, MPI_INT, 0, MPI_COMM_WORLD);

        *norm = globalsum / globaln;
      } 
    } END_COMPONENT_LOOP;
  } END_MAP_LOOP;
...
extern "C" void CT_Integral(CCTK_ARGUMENTS, const char *varName, CCTK_REAL *integ, const CCTK_INT refinement, const CCTK_INT nequation)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
...
CCTK_REAL CT_GetValue(CCTK_ARGUMENTS, const CCTK_REAL xcoord, const CCTK_REAL ycoord, const CCTK_REAL zcoord, const char *varName)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
...
extern "C" void CT_ClearFiles(CCTK_ARGUMENTS)
{
  ...
  for (int nequation=0; nequation < number_of_equations; nequation++)
  {
    ...
    sprintf(fullname, "%s/%s_eqn%d.asc", path, "respsi_norm", nequation);
    remove(fullname);
  }
