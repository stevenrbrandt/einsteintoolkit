                            ...
                            CCTK_INT enf_int)
{
  ...
  ENTER_LEVEL_MODE(cctkGH,level) {
    CCTK_INT step = 0;
    CCTK_REAL norm = 1e+03;
    std::vector<CCTK_REAL> value(number_of_equations);
...
//    if (init_coeff) CT_InitializeCoefficients(CCTK_PASS_CTOC);
    ...
    while (norm > tol && step < nsteps)
    {
      ...
      if (rset_psi > 0 && step%reset_every == 0)
      {
        ...
        CT_Reset(CCTK_PASS_CTOC, "CT_MultiLevel::ct_psi", &value[0], 0);
      }
      if (enf_int) CT_EnforceInt(CCTK_PASS_CTOC, &value[0]); // Is the conditional necessary?
      if (rset_psi < 0 && step%reset_every == 0)
      {
        CT_Reset(CCTK_PASS_CTOC, "CT_MultiLevel::ct_psi", &value[0], 1);
      }
      ...
      {
        CT_CalcPsiResidual(CCTK_PASS_CTOC, step, 0, &tmp, 0);
        CT_OutputWalk(CCTK_PASS_CTOC);
      }
      CT_CalcPsiResidual(CCTK_PASS_CTOC, step, 0, &norm, 0);
    }
    ...
    CT_CalcPsiResidual(CCTK_PASS_CTOC, step, 1, &norm, 0);
