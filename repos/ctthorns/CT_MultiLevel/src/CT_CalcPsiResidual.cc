...
extern "C" void CT_CalcPsiResidual(CCTK_ARGUMENTS, CCTK_INT step, CCTK_INT output, CCTK_REAL *norm, CCTK_INT outdated_psi)
{
  ...
  *norm = 0;
  ...
  for (int nequation=0; nequation < number_of_equations; nequation++)
  {
    ...
    char name[100];
    sprintf(name, "respsi_norm_eqn%d.asc", nequation);
    if (!output && CCTK_Equals(output_norms,"yes")) CT_WriteTimeSeries(step, eqnnorm, name);
  } // for
  return;
}
