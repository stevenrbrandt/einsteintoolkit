...
static void CT_Dust_addToTmunu_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL eTttL CCTK_ATTRIBUTE_UNUSED = (prsL - 
      0.0397887357729738339422209408431*Lambda)*(bsq - pow(alpL,2)) + (prsL + 
      rhoL)*pow(u0l,2);
    
    CCTK_REAL eTtxL CCTK_ATTRIBUTE_UNUSED = (betaxL*gxxL + betayL*gxyL + 
      betazL*gxzL)*(prsL - 0.0397887357729738339422209408431*Lambda) + (prsL 
      + rhoL)*u1L*u0l;
    
    CCTK_REAL eTtyL CCTK_ATTRIBUTE_UNUSED = (betaxL*gxyL + betayL*gyyL + 
      betazL*gyzL)*(prsL - 0.0397887357729738339422209408431*Lambda) + (prsL 
      + rhoL)*u2L*u0l;
    
    CCTK_REAL eTtzL CCTK_ATTRIBUTE_UNUSED = (betaxL*gxzL + betayL*gyzL + 
      betazL*gzzL)*(prsL - 0.0397887357729738339422209408431*Lambda) + (prsL 
      + rhoL)*u3L*u0l;
    
    CCTK_REAL eTxxL CCTK_ATTRIBUTE_UNUSED = gxxL*(prsL - 
      0.0397887357729738339422209408431*Lambda) + (prsL + rhoL)*pow(u1L,2);
    
    CCTK_REAL eTxyL CCTK_ATTRIBUTE_UNUSED = (prsL + rhoL)*u1L*u2L + 
      gxyL*(prsL - 0.0397887357729738339422209408431*Lambda);
    
    CCTK_REAL eTxzL CCTK_ATTRIBUTE_UNUSED = (prsL + rhoL)*u1L*u3L + 
      gxzL*(prsL - 0.0397887357729738339422209408431*Lambda);
    
    CCTK_REAL eTyyL CCTK_ATTRIBUTE_UNUSED = gyyL*(prsL - 
      0.0397887357729738339422209408431*Lambda) + (prsL + rhoL)*pow(u2L,2);
    
    CCTK_REAL eTyzL CCTK_ATTRIBUTE_UNUSED = (prsL + rhoL)*u2L*u3L + 
      gyzL*(prsL - 0.0397887357729738339422209408431*Lambda);
    
    CCTK_REAL eTzzL CCTK_ATTRIBUTE_UNUSED = gzzL*(prsL - 
  ...
  CCTK_ENDLOOP3(CT_Dust_addToTmunu);
}
extern "C" void CT_Dust_addToTmunu(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CT_Dust_addToTmunu_Body");
  }
  if (cctk_iteration % CT_Dust_addToTmunu_calc_every != CT_Dust_addToTmunu_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "CT_Dust::CT_rho",
    "CT_Dust::CT_u"};
  AssertGroupStorage(cctkGH, "CT_Dust_addToTmunu", 6, groups);
  ...
  LoopOverEverything(cctkGH, CT_Dust_addToTmunu_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CT_Dust_addToTmunu_Body");
  }
}
