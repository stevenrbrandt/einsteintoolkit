...
static void CT_Dust_InitRHS_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_ENDLOOP3(CT_Dust_InitRHS);
}
extern "C" void CT_Dust_InitRHS(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CT_Dust_InitRHS_Body");
  }
  if (cctk_iteration % CT_Dust_InitRHS_calc_every != CT_Dust_InitRHS_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "CT_Dust::CT_Erhs",
    "CT_Dust::CT_Srhs"};
  AssertGroupStorage(cctkGH, "CT_Dust_InitRHS", 3, groups);
  ...
  LoopOverEverything(cctkGH, CT_Dust_InitRHS_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CT_Dust_InitRHS_Body");
  }
}
