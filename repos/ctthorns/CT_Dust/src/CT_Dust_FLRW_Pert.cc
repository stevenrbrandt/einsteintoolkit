...
static void CT_Dust_FLRW_Pert_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL HH CCTK_ATTRIBUTE_UNUSED = pflrw_H0*pflrw_t0*pow(t,-1);
    
    CCTK_REAL aa CCTK_ATTRIBUTE_UNUSED = 
      pflrw_a0*pow(t*pow(pflrw_t0,-1),0.666666666666666666666666666667);
    
    CCTK_REAL rhocrit CCTK_ATTRIBUTE_UNUSED = 
      0.119366207318921501826662822529*pow(HH,2);
    
    rhoL = pflrw_omegaM*rhocrit*(1 + (pflrw_ax[0])*sin(pflrw_kx[0]*xL) + 
      (pflrw_ax[1])*sin(pflrw_kx[1]*xL) + (pflrw_ax[10])*sin(pflrw_kx[10]*xL) 
      + (pflrw_ax[11])*sin(pflrw_kx[11]*xL) + 
      (pflrw_ax[12])*sin(pflrw_kx[12]*xL) + 
      (pflrw_ax[13])*sin(pflrw_kx[13]*xL) + 
      (pflrw_ax[14])*sin(pflrw_kx[14]*xL) + 
      (pflrw_ax[15])*sin(pflrw_kx[15]*xL) + 
      (pflrw_ax[16])*sin(pflrw_kx[16]*xL) + 
      (pflrw_ax[17])*sin(pflrw_kx[17]*xL) + 
      (pflrw_ax[18])*sin(pflrw_kx[18]*xL) + 
      (pflrw_ax[19])*sin(pflrw_kx[19]*xL) + (pflrw_ax[2])*sin(pflrw_kx[2]*xL) 
      + (pflrw_ax[3])*sin(pflrw_kx[3]*xL) + (pflrw_ax[4])*sin(pflrw_kx[4]*xL) 
      + (pflrw_ax[5])*sin(pflrw_kx[5]*xL) + (pflrw_ax[6])*sin(pflrw_kx[6]*xL) 
      + (pflrw_ax[7])*sin(pflrw_kx[7]*xL) + (pflrw_ax[8])*sin(pflrw_kx[8]*xL) 
      + (pflrw_ax[9])*sin(pflrw_kx[9]*xL) + (pflrw_ay[0])*sin(pflrw_ky[0]*yL) 
      + (pflrw_ay[1])*sin(pflrw_ky[1]*yL) + 
      (pflrw_ay[10])*sin(pflrw_ky[10]*yL) + 
      (pflrw_ay[11])*sin(pflrw_ky[11]*yL) + 
      (pflrw_ay[12])*sin(pflrw_ky[12]*yL) + 
      (pflrw_ay[13])*sin(pflrw_ky[13]*yL) + 
      (pflrw_ay[14])*sin(pflrw_ky[14]*yL) + 
      (pflrw_ay[15])*sin(pflrw_ky[15]*yL) + 
      (pflrw_ay[16])*sin(pflrw_ky[16]*yL) + 
      (pflrw_ay[17])*sin(pflrw_ky[17]*yL) + 
      (pflrw_ay[18])*sin(pflrw_ky[18]*yL) + 
      (pflrw_ay[19])*sin(pflrw_ky[19]*yL) + (pflrw_ay[2])*sin(pflrw_ky[2]*yL) 
      + (pflrw_ay[3])*sin(pflrw_ky[3]*yL) + (pflrw_ay[4])*sin(pflrw_ky[4]*yL) 
      + (pflrw_ay[5])*sin(pflrw_ky[5]*yL) + (pflrw_ay[6])*sin(pflrw_ky[6]*yL) 
      + (pflrw_ay[7])*sin(pflrw_ky[7]*yL) + (pflrw_ay[8])*sin(pflrw_ky[8]*yL) 
      + (pflrw_ay[9])*sin(pflrw_ky[9]*yL) + (pflrw_az[0])*sin(pflrw_kz[0]*zL) 
      + (pflrw_az[1])*sin(pflrw_kz[1]*zL) + 
      (pflrw_az[10])*sin(pflrw_kz[10]*zL) + 
      (pflrw_az[11])*sin(pflrw_kz[11]*zL) + 
      (pflrw_az[12])*sin(pflrw_kz[12]*zL) + 
      (pflrw_az[13])*sin(pflrw_kz[13]*zL) + 
      (pflrw_az[14])*sin(pflrw_kz[14]*zL) + 
      (pflrw_az[15])*sin(pflrw_kz[15]*zL) + 
      (pflrw_az[16])*sin(pflrw_kz[16]*zL) + 
      (pflrw_az[17])*sin(pflrw_kz[17]*zL) + 
      (pflrw_az[18])*sin(pflrw_kz[18]*zL) + 
      (pflrw_az[19])*sin(pflrw_kz[19]*zL) + (pflrw_az[2])*sin(pflrw_kz[2]*zL) 
      + (pflrw_az[3])*sin(pflrw_kz[3]*zL) + (pflrw_az[4])*sin(pflrw_kz[4]*zL) 
      + (pflrw_az[5])*sin(pflrw_kz[5]*zL) + (pflrw_az[6])*sin(pflrw_kz[6]*zL) 
      + (pflrw_az[7])*sin(pflrw_kz[7]*zL) + (pflrw_az[8])*sin(pflrw_kz[8]*zL) 
      + (pflrw_az[9])*sin(pflrw_kz[9]*zL));
    
    CCTK_REAL rhodpL CCTK_ATTRIBUTE_UNUSED = rhoL*pow(aa,3);
    ...
    CCTK_REAL a2 CCTK_ATTRIBUTE_UNUSED = pow(aa,2);
    
    CCTK_REAL gxxL CCTK_ATTRIBUTE_UNUSED = a2;
    
    CCTK_REAL gxyL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL gxzL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL gyyL CCTK_ATTRIBUTE_UNUSED = a2;
    
    CCTK_REAL gyzL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL gzzL CCTK_ATTRIBUTE_UNUSED = a2;
    
    CCTK_REAL kxxL CCTK_ATTRIBUTE_UNUSED = -(a2*HH);
    
    CCTK_REAL kxyL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL kxzL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL kyyL CCTK_ATTRIBUTE_UNUSED = -(a2*HH);
    
    CCTK_REAL kyzL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL kzzL CCTK_ATTRIBUTE_UNUSED = -(a2*HH);
    /* Copy local copies back to grid functions */
    eps[index] = epsL;
    gxx[index] = gxxL;
    gxy[index] = gxyL;
  ...
  CCTK_ENDLOOP3(CT_Dust_FLRW_Pert);
}
extern "C" void CT_Dust_FLRW_Pert(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering CT_Dust_FLRW_Pert_Body");
  }
  if (cctk_iteration % CT_Dust_FLRW_Pert_calc_every != CT_Dust_FLRW_Pert_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "CT_Dust::CT_u",
    "grid::coordinates"};
  AssertGroupStorage(cctkGH, "CT_Dust_FLRW_Pert", 8, groups);
  ...
  LoopOverEverything(cctkGH, CT_Dust_FLRW_Pert_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving CT_Dust_FLRW_Pert_Body");
  }
}
