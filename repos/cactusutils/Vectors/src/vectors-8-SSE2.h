...
#define vec8_architecture                                                      \
  "SSE2" vec8_architecture_SSE4_1 vec8_architecture_SSE4a                      \
      vec8_architecture_FMA4 " (64-bit precision)"
...
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC
vec8_set(CCTK_REAL8 const a, CCTK_REAL8 const b) {
  return _mm_set_pd(b, a); // note reversed arguments
}

// original order is 01
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC
vec8_swap10(CCTK_REAL8_VEC const x) {
...
vec8_elti(CCTK_INTEGER8_VEC const x, std::ptrdiff_t const d) {
  CCTK_INTEGER8 e;
  std::memcpy(&e, &((char const *)&x)[d * sizeof e], sizeof e);
  return e;
}
...
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC
vec8_load_off1(CCTK_REAL8 const &p) {
  return vec8_loadu(p);
}
#else
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC
vec8_load_off1(CCTK_REAL8 const &p) {
  return _mm_shuffle_pd(vec8_load((&p)[-1]), vec8_load((&p)[+1]),
                        _MM_SHUFFLE2(0, 1));
}
#endif
...
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE void
vec8_store_partial_prepare_(bool &lo, bool &hi, std::ptrdiff_t const i,
...
k8copysign(CCTK_REAL8_VEC const x, CCTK_REAL8_VEC const y) {
  return _mm_or_pd(_mm_and_pd(I2R(k8notsign), x), _mm_and_pd(I2R(k8sign), y));
}
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC
...
#define K8REPL(f, x) vec8_set(f(vec8_elt(x, 0)), f(vec8_elt(x, 1)));
#define K8REPL2S(f, x, a) vec8_set(f(vec8_elt(x, 0), a), f(vec8_elt(x, 1), a));
...
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC k8ifthen(
  ...
  case 0:
    return y;
  case 1:
    return _mm_move_sd(y, z);
  case 2:
    return _mm_move_sd(z, y);
  }
  return z;
#elif 0
  return vec8_set(
      std::signbit(vec8_elt(x, 0)) ? vec8_elt(y, 0) : vec8_elt(z, 0),
      std::signbit(vec8_elt(x, 1)) ? vec8_elt(y, 1) : vec8_elt(z, 1));
#elif 0
  // We don't need to shift -- the condition (mask) will be either all
  // zeros or all ones
  k8const_t const k8ione = {{
      1, 1,
  }};
