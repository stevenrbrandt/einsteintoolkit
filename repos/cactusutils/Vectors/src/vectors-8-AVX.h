...
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE CCTK_REAL8_VEC
vec8_set(CCTK_REAL8 const a, CCTK_REAL8 const b, CCTK_REAL8 const c,
         CCTK_REAL8 const d) {
...
static inline CCTK_ATTRIBUTE_ALWAYS_INLINE void
vec8_store_partial_prepare_fixed_(bool &all, __m256i &mask,
...
vec8_store_nta_partial_mid(CCTK_REAL8 &p, CCTK_REAL8_VEC const x,
                           ptrdiff_t const nlo, ptrdiff_t const nhi) {
  _mm256_maskstore_pd(
      &p, R2I(_mm256_and_pd(I2R(k8store_lo[nlo].vi), I2R(k8store_hi[nhi].vi))),
      x);
}
#endif
