...
void Convert_GiRaFFE_to_HydroBase(CCTK_ARGUMENTS) {
  ...
  if(Convert_GiRaFFE_to_HydroBase_every==0 || cctk_iteration%Convert_GiRaFFE_to_HydroBase_every!=0) return;
    ...
    for(int j=0;j<cctk_lsh[1];j++)
      for(int i=0;i<cctk_lsh[0];i++) {
        ...
        if(one_minus_one_over_alpha_u0_squared > 1.0) {
          CCTK_VInfo(CCTK_THORNSTRING,"Convert_GiRaFFE_to_HydroBase WARNING: Found superluminal velocity. This should have been caught by GiRaFFE.");
        }
        ...
        if(std::isnan(alpha_u0*lapseL_inv)) printf("BAD FOUND NAN ALPHAU0 CALC: %.15e %.15e %.15e | %.15e %.15e %.15e %.15e\n",alpha_u0,lapseL_inv,one_minus_one_over_alpha_u0_squared,PRIMS[VX],PRIMS[VY],PRIMS[VZ],shiftxL);
